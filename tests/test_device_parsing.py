...
async def test_mock_client_parsing(available_mock_devices):
    ...
    for device_name in available_mock_devices:
        ...
        print(
            f"Device {device_name}: {len(features)} feature models -> {flat_count} flat features"
        )
...
async def test_feature_filtering_and_expansion():
    ...
    assert "heating.operating" not in flat_names, (
        "Empty structural feature was not filtered out"
    )
    ...
    schedule_feat = next(
        (f for f in flat_features if f.name == "heating.circuits.0.heating.schedule"),
        None,
    )
...
async def test_redundant_suffix_removal():
    ...
    assert "heating.circuits.0.name.name" not in flat_names, (
        "Redundant name.name feature present"
    )
    ...
    for device_name in available_mock_devices:
        ...
        for f in features:
            ...
            if not has_value and not can_expand:
                problematic.append(
                    f"{device_name}:{f.name} (props: {list(f.properties.keys())})"
                )
