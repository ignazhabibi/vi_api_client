...
@pytest.fixture
def available_fixture_devices(device_responses_dir):
    """Return a list of available fixture device filenames (without extension)."""
    files = [f for f in os.listdir(device_responses_dir) if f.endswith(".json")]
    return sorted([os.path.splitext(f)[0] for f in files])


@pytest.fixture
def load_fixture_device(device_responses_dir):
    """Factory to load a fixture device JSON by name."""

    def _load(name):
        with open(os.path.join(device_responses_dir, f"{name}.json")) as f:
            return json.load(f)
